...
vector<double> joints_last_position{ 0, 0 };
...
void joint_states_callback(const sensor_msgs::JointState js)
{
    ...
    vector<double> joints_current_position = js.position;
...
int main(int argc, char** argv)
{
    ...
    Subscriber sub1 = n.subscribe("/simple_arm/joint_states", 10, joint_states_callback);

    // Subscribe to rgb_camera/image_raw topic to read the image data inside the look_away_callback function
    Subscriber sub2 = n.subscribe("rgb_camera/image_raw", 10, look_away_callback);
