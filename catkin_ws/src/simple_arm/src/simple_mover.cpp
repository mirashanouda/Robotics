...
int main(int argc, char** argv)
{
    ...
    Publisher joint1_pub = n.advertise<Float64>("/simple_arm/joint_1_position_controller/command", 10);
    ...
    Publisher joint2_pub = n.advertise<Float64>("/simple_arm/joint_2_position_controller/command", 10);
    ...
    while (not start_time) {
        start_time = Time::now().toSec();
    }

    while (ros::ok()) {
        // Get ROS elapsed time
        elapsed = Time::now().toSec() - start_time;
